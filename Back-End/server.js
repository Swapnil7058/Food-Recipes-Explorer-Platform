...
function loadRecipesFromCsv() {
    return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(CSV_FILE_PATH)
            .pipe(csv())
            .on('data', (data) => {
                const recipeTitle = data.TranslatedRecipeName || data.RecipeName || 'Unknown Dish';
                ...
                const instructionsString = data.TranslatedInstructions || data.Instructions || "";
            ...
            .on('end', () => {
                ...
                console.log(`Successfully loaded ${recipesDb.length} recipes from CSV.`);
                resolve();
            })
            .on('error', (error) => {
...
app.get('/api/recipes', (req, res) => {
    ...
    if (recipesDb.length === 0) {
        return res.status(500).json({ message: "Recipe database is not loaded on the server." });
    }
    ...
    if (isFavorite) {
        ...
        if (recipeToAdd && recipeToAdd.id) {
            ...
            if (fullRecipe) {
             ...
             res.status(400).json({ message: 'Invalid recipe data provided (missing ID).' });
        }
    }
});
